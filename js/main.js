...
(function() {
	...
	function initEvents() {
		[].slice.call(gridItems).forEach(function(item, pos) {
			...
			item.addEventListener('click', function(ev) {
				...
				classie.add(item, 'grid__item--animate');
	       		item.id && history.pushState(null, '', '#' + item.id);
				loadContent(item);
			});
		});
		...
		const itemIndex = Object.values(gridItems).findIndex(i =>
			i.id == window.location.hash.substr(1)
		)
		if (itemIndex >= 0) current = itemIndex

		const hashId = window.location.hash.substr(1)
		if (!!hashId) {
			const item = Object.values(gridItems).find(i => i.id === hashId)
			item && loadContent(item)
		}
